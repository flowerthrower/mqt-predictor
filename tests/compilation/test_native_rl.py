...
def test_parse_sampled_native_episode() -> None:
    ...
    trace = """
[mqt-predictor] step=0 action=merge-rotations qubits=4 depth=8 two_qubit_depth=2 gates=12 two_qubit=3 mapped=0 routed=0 synthesized=0 legal=111110 features={relative_qubits=1,log_depth=0.2,program_communication=0.5,critical_depth=0.4,entanglement_ratio=0.25,parallelism=0.5,liveness=0.75}
[mqt-predictor] step=1 action=merge-rotations qubits=4 depth=7 two_qubit_depth=2 gates=11 two_qubit=3 mapped=0 routed=0 synthesized=0 legal=111110 features={relative_qubits=1,log_depth=0.19,program_communication=0.5,critical_depth=0.4,entanglement_ratio=0.27,parallelism=0.5,liveness=0.75}
[mqt-predictor] step=2 action=terminate qubits=4 depth=10 two_qubit_depth=4 gates=16 two_qubit=6 mapped=1 routed=1 synthesized=1 legal=000001 features={relative_qubits=1,log_depth=0.22,program_communication=0.5,critical_depth=0.67,entanglement_ratio=0.38,parallelism=0.4,liveness=0.6}
"""
    ...
    assert episode.action_counts == {"merge-rotations": 2, "terminate": 1}
    assert episode.actions == ("merge-rotations", "merge-rotations", "terminate")
...
def test_parse_exhaustive_single_use_result() -> None:
    ...
    trace = """
[mqt-predictor] search-result winner=7 schedule=merge-rotations>fuse-two-qubit valid=17 unique_outputs=14 two_qubit_depth=4 two_qubit=6 depth=10 gates=16 total_compile_us=42
"""
...
def test_terminal_reward_uses_core_relative_quality_and_pass_cost() -> None:
    ...
    pass_transition = Transition(
        action=ACTION_NAMES.index("merge-rotations"),
        features=(0.5,) * len(FEATURE_NAMES),
...
def test_reinforce_increases_positive_action_logit() -> None:
    ...
    legal = (True, True, False, False, False, False)
